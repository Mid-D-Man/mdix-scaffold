...
**The moment `@QUICKFUNCS` goes from empty/absent to containing something, `@CONFIG`'s `features` has to say so too** — `features -> "quickfuncs,data"`, not `"data"`. This is a hard compile error (`@QUICKFUNCS is not allowed with current feature settings`), not a silent no-op, and it's the single most common mistake when editing an existing template: the person adds a function, forgets the one-word `features` update, and the whole file fails to parse despite the new function itself being flawless. See the writing skill's `@CONFIG` section for the full detail — the short version is: every section add/remove is a two-line edit (the section, and `features`), never a one-line edit.
...
**Archives:** any `.tar.gz`, `.tgz`, `.tar`, or `.zip` file anywhere in the replacements tree (flat or nested) is *not* copied as a literal file — it's extracted into a temporary staging area at the exact position it occupies, and its contents are then treated as ordinary candidates using the same flat/nested resolution rules above, recursively (an archive containing another archive gets extracted again). This exists for delivering a whole replacement set as one file where committing dozens of individual files isn't practical — a mobile client with no local shell being the motivating case: commit `replacements.mdix` and one `.tar.gz` sitting next to it through a web UI, nothing else, then run the workflow. By default, the archive itself is never modified or deleted from `replacements/` (extraction happens in a throwaway temp copy, not in place) — re-running later re-extracts from the same still-present archive, and a dry run never touches disk beyond that temp copy either. **Opt-in exception:** pass `delete_processed_archives: true` (workflow input, plumbed to `DELETE_PROCESSED_ARCHIVES` env / `--delete-processed-archives` flag) to delete a top-level archive from its real location in `replacements/` once a run completes with zero errors and isn't a dry run — trades away the "still-present, re-extracts safely" default specifically to prevent an archive from being accidentally re-applied by a later run. A dry run or a run with any errors never deletes anything regardless of this setting, and only archives that were genuinely sitting in `replacements/` are eligible — one extracted from *inside* another archive (nested archive-within-archive) is staging-only and was never a real file there to delete. Archive contents are treated as untrusted input: every member is checked for path-traversal (absolute paths, `../` escapes) and symlinks *before* anything is extracted, and the whole archive is rejected with nothing written if any member fails that check — not a partial extraction of only the safe-looking members.

**Flat vs. nested inside an archive is determined the exact same way as outside one — by the archive's own top level, not the archive's position in `replacements/`.** Verified straight from the resolver (`lib_replacements.py`: `is_nested = "/" in rel_path`, checked against each extracted member's path *relative to the archive's own root*). Concretely: an archive sitting flat in `replacements/` that contains `Cargo.toml` at its own top level produces a **flat** candidate (`rel_path == "Cargo.toml"`, no `/`) — resolved by basename search across the *entire* `target_root`, not by direct path — even though the archive as a whole is "one file" and feels like it should behave as a unit. Only members that sit inside a subdirectory *within the archive* (`crates/mid-qr-core/src/generate.rs`) get the nested direct-path treatment. This is easy to get backwards: packing a batch of real repo files into an archive and assuming every one of them is safely pinned by its path, when only the ones with a subdirectory component actually are. Any genuinely repo-root file bundled this way (`Cargo.toml`, `README.md`, `package.json`, `LICENSE`, ...) needs its own `overrides::` entry — don't rely on it happening to be unique across `target_root` today. A workspace with one `Cargo.toml` per crate is exactly the shape that turns this into a hard ambiguity error the moment there's more than one.
...
- **Assuming a file is "nested" (path-pinned, no search) just because it travels inside an archive.** Only members sitting in a subdirectory *within the archive itself* get that treatment — anything at the archive's own top level is flat, resolved by basename search across the whole `target_root`, identically to a flat file sitting directly next to the manifest. Bundling real repo-root files (`Cargo.toml`, `README.md`, `package.json`, ...) at an archive's top level and trusting the archive-as-a-unit to keep them safe is exactly how a 3-way `Cargo.toml` collision (workspace root + two per-crate manifests) turns into a hard error — give each of those an explicit `overrides::` entry instead of assuming they're unique.
